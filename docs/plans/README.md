...
| [ai-driving-fixes.md](ai-driving-fixes.md) | Four defects a player's 44-minute expert diagnostic exposed, each of which stopped an AI unit permanently: a one-unit-wide dead band where arrival was measured to the dock and release to the building centre (two teams finished on 320cr and 0cr because of it); `_driveOneScout` having no `repair` guard, so aiCommander and repairController steered the same scout and neither's give-up was ever reachable (detour counts of 74 against a six-angle ladder); `markDocked` stopping the clearance lease one leg before service actually starts (a scout held a bay for 372 seconds from 228 units away); and `combatCap` being a per-unit-id allowance, so every AI bought 7 scouts it then refused to field. Re-run confirms all four; a fifth, separately root-caused, is recorded and left for its owner's call. |
